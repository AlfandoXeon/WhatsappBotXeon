...
async function handleTikTok(sock, msg, remoteJid, sender, pushName, args) {
    const urlText = args.join(' ').trim();
    const url = extractUrl(urlText);
    
    if (!url || (!url.includes('tiktok.com'))) {
        return await sock.sendMessage(remoteJid, { text: "❌ Tautan TikTok tidak valid! Contoh: `#tt https://vt.tiktok.com/...`" }, { quoted: msg.fakeReply || msg });
    }

    try {
        await sock.sendMessage(remoteJid, { text: "🔍 Mengekstrak media TikTok..." }, { quoted: msg.fakeReply || msg });
        logTable('INFO', pushName, `Mengambil info TikTok: ${url}`);
        
        const info = await getVideoInfo(url);
        
        // Tentukan jika ini carousel (memiliki banyak gambar)
        const isCarousel = info._images && info._images.length > 0;
        
        const infoText = `📋 *TIKTOK DOWNLOAD* 📋\n\n` +
            `*Judul:* ${info.title || '-'}\n` +
            `*Uploader:* ${info.uploader || '-'}\n` +
            `*Tipe:* ${isCarousel ? `Carousel Foto (${info._images.length} item)` : 'Video HD'}\n\n` +
        ...
        addToQueue({
            ...
            title: info.title || 'TikTok_XeonBot',
            info: info
        });

    } catch (err) {
        logTable('ERROR', pushName, `TikTok Handler error: ${err.message}`);
